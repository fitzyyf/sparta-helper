...
public class ResourceURL {
     ...
     * of the {@link java.util.jar.JarFile} API.  Only jar:// and file:// URLs are supported.
     *
    ...
    public static boolean isDirectory(URL resourceURL) throws URISyntaxException {
        ...
        } else if (StringUtils.equals("file", protocol)) {
    ...
    /**
     * Appends a trailing '/' to a {@link java.net.URL} object. Does not append a slash if one is already present.
     *
     ...
     * 1) calling getConnection on a {@link java.net.URLConnection} to a file opens an {@link java.io.InputStream} to that file that must
     * then be closed — though this is not true for {@code URLConnection}s to jar resources
     * 2) calling getLastModified on {@link java.net.JarURLConnection}s returns the last modified time of the jar file, rather
